...
m = 258.612; % sprung mass [kg]
mF = 10.347; % half front unsprung mass [kg]
mR = 10.347; % half rear unsprung mass [kg]

hcog = 0.3054; % height of center of gravity of sprung mass [m]
...
lR = l-lF; % distance of the rear axle to the center of gravity [m]         TO DO
wF = 1.578; % front track [m]                                        
wR = 1.544; % rear track [m]


hpc = 0.0; % height of pitch center [m]                                     TO DO
hrcF = 0; % height of the roll center at the front axle [m]                 TO DO
hrcR = 0; % height of the roll center at the rear axle [m]                  TO DO

kF = 40000; % front springs rate [N/m]
kR = 35000; % rear springs rate [N/m]

cF = 1000; % Damping rate at the front [Ns/m]                               TO DO
cR = 1000; % Damping rate at the rear [Ns/m]                                TO DO

kRF = 0; % Front antiroll bar rate [N/m] or [Nm/rad]                        TO DO
kRR = 0; % Rear antiroll bar rate [N/m] or [Nm/rad]                         TO DO

% Aerodynamics 
Aero_balance=0.52; % percent of rear downforce
cl=-2.8; % total cl
clF = -cl*(1-Aero_balance); % Front lift coefficient (negative creates downforce)
clR = -cl*Aero_balance; % Rear lift coefficient  (negative creates downforce)
cd = 1.8; % Drag coefficient
A = 1; % Vehicle's frontal area [m^2]
rho = 1.225; % Air density [kg/m^3]

steer_ratio = 13.25; % Ratio between steering wheel and wheel toe angle     TO DO
...
%Engine/Motors Maps
Engine_Torque = readmatrix('EngineMap.xlsx','Range','A2:A20');
Engine_Speed = readmatrix('EngineMap.xlsx','Range','B2:B20');
Throttle_Position = readmatrix('EngineMap.xlsx','Range','C2:C20');
...
enableDriverControl = 1; %% If disabled the “driver” will not track the requested speed but the user can introduce external throttle and brake inputs in the "driver block"
...
kappa_upper_limit=0.15;%setting upper limit for driver long slip controll
